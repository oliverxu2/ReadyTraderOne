...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            new_bid_price = bid_prices[0] - int(self.position/(POSITION_LIMIT/3)) * TICK_SIZE_IN_CENTS if bid_prices[0] != 0 else 0
            new_ask_price = ask_prices[0] - int(self.position/(POSITION_LIMIT/3)) * TICK_SIZE_IN_CENTS if ask_prices[0] != 0 else 0
